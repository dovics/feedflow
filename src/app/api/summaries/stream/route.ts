...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.log('[Stream Summary] Auth check:', Date.now() - startTime, 'ms');
    ...
    if (!config) {
      return new Response(
        JSON.stringify({ error: "未配置 OpenAI API，请在管理员设置中配置" }),
    ...
    console.log('[Stream Summary] Got config:', Date.now() - startTime, 'ms');
    ...
    console.log('[Stream Summary] Database query took:', Date.now() - queryStart, 'ms, found items:', items.length);
    ...
    // Group items by tags and build article link map
    ...
    sortedTags.forEach((tag) => {
      groupedText += `\n## ${tag}\n`;
      grouped[tag].slice(0, 5).forEach((item) => {
        if (item.link) {
    ...
    if (untagged.length > 0) {
      groupedText += "\n## 其他\n";
      untagged.slice(0, 5).forEach((item) => {
        if (item.link) {
          articleLinkMap[globalIndex] = item.link;
        }
        groupedText += `[${globalIndex}] ${item.title}\n`;
...
Keep it concise, limit each section to 2-3 sentences. Use markdown format.`;
    ...
    console.log('[Stream Summary] Data processing took:', Date.now() - processStart, 'ms');
    console.log('[Stream Summary] Total time before streamText:', Date.now() - startTime, 'ms');
    ...
    return result.toTextStreamResponse({
      headers: {
        'X-Article-Map': JSON.stringify(articleLinkMap),
